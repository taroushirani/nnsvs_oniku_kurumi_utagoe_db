...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    python utils/gen_lab.py
    python utils/align_lab.py
    python utils/perf_segmentation.py
    python utils/finalize_lab.py
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    xrun nnsvs-train data.train_no_dev.in_dir=$dump_norm_dir/$train_set/in_acoustic/ \
        ...
        data.batch_size=$config_batch_size \
        resume.checkpoint=$resume_checkpoint \
	model.stream_sizes=$config_acoustic_model_stream_sizes \
        model.netG.params.out_dim=$config_acoustic_model_out_dim
fi
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
    ...
    for s in ${testsets[@]}; do
        for input in label_phone_score label_phone_align; do
            ...
            xrun python bin/synthesis_nsf.py question_path=$config_question_path \
            ...
            acoustic.stream_sizes=$config_acoustic_model_stream_sizes \
	    acoustic.post_filter=false \
