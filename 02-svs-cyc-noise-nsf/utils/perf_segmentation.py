...
for name in ["full_dtw", "sinsy_full_round", "sinsy_mono_round"]:
    ...
    for idx, base in tqdm(enumerate(base_files)):
        ...
        base_segments, start_indices, end_indices = segment_labels(
            base_lab, True, float(config["segmentation_threshold"]),
            min_duration=float(config["segment_min_duration"]),
            force_split_threshold=float(config["force_split_threshold"]))
        if name == "full_dtw":
            d = []
